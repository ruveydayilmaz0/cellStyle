    ...
    def forward(self, x):
        ...
        for l in range(L):
            ...
            diff_u0 = abs(
                Iu0
                - torch.unsqueeze(
                    torch.tensor(np.linspace(-3, 3, num=self.h)), dim=0
                ).to(self.device)
            )
            diff_v0 = abs(
                Iv0
            ...
            if self.method == "thresholding":
                ...
                diff_u0 = (
                    torch.pow(torch.reshape(diff_u0, (-1, self.h)), 2) / self.sigma**2
                )
                diff_v0 = (
                    torch.pow(torch.reshape(diff_v0, (-1, self.h)), 2) / self.sigma**2
                )
            ...
            if self.method == "thresholding":
                ...
                diff_u2 = (
                    torch.pow(torch.reshape(diff_u2, (-1, self.h)), 2) / self.sigma**2
                )
                diff_v2 = (
                    torch.pow(torch.reshape(diff_v2, (-1, self.h)), 2) / self.sigma**2
                )
...
histogram_block = RGBuvHistBlock(
    insz=max_input_size,
    h=histogram_size,
...
if len(sty_list) != len(tar_list):
    print(
        "The number of stylized and style images are not matched!!",
        f"sty: {len(sty_list)}, tar: {len(tar_list)}",
    )
    exit()
hist_list = []
