    ...
    def forward(self, x, y):
        if self.metric_type == "ssim" or self.metric_type == "ms-ssim":
            ...
            for layer in features_x.keys():
                dist += torch.mean(
                    torch.square(features_x[layer] - features_y[layer]), dim=(1, 2, 3)
                )
...
class GramLoss(nn.Module):
    ...
    def gram_matrix(self, input):
        ...
        features = input.view(a, 1, b * c * d).div(
            math.sqrt(b * c * d)
    ...
    def loss_from_feat(self, input_feats, target_feats):
        ...
        for idx in range(len(input_feats)):
            loss = F.mse_loss(
                self.gram_matrix(input_feats[idx]), self.gram_matrix(target_feats[idx])
            )
            init_loss += loss
        return init_loss
    ...
    def forward(self, input, target):
        ...
        for idx in range(len(src_feats)):
            loss = F.mse_loss(
                self.gram_matrix(src_feats[idx]), self.gram_matrix(target_feats[idx])
            )
            init_loss += loss
        init_loss /= 5
        return init_loss
