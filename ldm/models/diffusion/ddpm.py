...
class LatentDiffusion(DDPM):
    ...
    def decode_first_stage(
        self, z, predict_cids=False, force_not_quantize=False, return_feat=False
    ):
        if predict_cids:
            if z.dim() == 4:
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, injected_features=None):
        ...
        if hasattr(self, "split_input_params"):
            ...
            output_list = [
                self.model(
                    z_list[i], t, **cond_list[i], injected_features=injected_features
                )
                for i in range(z.shape[-1])
            ]
            ...
            x_recon = self.model(
                x_noisy, t, **cond, injected_features=injected_features
            )
    ...
    def forward(
        ...
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(
                x, t, context=cc, injected_features=injected_features
            )
        elif self.conditioning_key == "hybrid":
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(
                xc, t, context=cc, injected_features=injected_features
            )
