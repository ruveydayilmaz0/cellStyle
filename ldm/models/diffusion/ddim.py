...
class DDIMSampler(object):
    ...
    def make_negative_prompt_schedule(
        self, negative_prompt_schedule, negative_prompt_alpha, total_steps
    ):
    ...
    def sample(
        ...
        **kwargs,
        ...
        self.make_schedule(
            ddim_num_steps=S, ddim_eta=eta, verbose=verbose, strength=strength
        )
        # sampling
        C, H, W = shape
    ...
    def ddim_sampling(
        ...
        callback_ddim_timesteps_list = (
            np.flip(
                make_ddim_timesteps(
                    "uniform", callback_ddim_timesteps, self.ddpm_num_timesteps
                )
            )
        ...
        negative_prompt_alpha_schedule = self.make_negative_prompt_schedule(
            negative_prompt_schedule, negative_prompt_alpha, total_steps
        )
        style_loss = None
    ...
    def p_sample_ddim(
        ...
        if negative_conditioning is not None:
            ...
            e_t_negative, e_t_uncond = self.model.apply_model(
                x_in, t_in, c_in, injected_features=injected_features
            ).chunk(2)
            ...
            e_t_tilde = (
                negative_prompt_alpha * e_t_uncond
                + (1 - negative_prompt_alpha) * e_t_negative
            )
    ...
    def reverse_ddim(
        ...
        a_next = torch.full(
            (b, 1, 1, 1), alphas[t_next], device=device
        )  # a_next = torch.full((b, 1, 1, 1), alphas[t + 1], device=device)
        sqrt_one_minus_at = torch.full(
            (b, 1, 1, 1), sqrt_one_minus_alphas[t], device=device
        )
